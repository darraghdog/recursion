...
do
    bsub  -q lowpriority -gpu "num=$N_GPU:mode=exclusive_process" -app gpu -n =$N_GPU  -env LSB_CONTAINER_IMAGE=darraghdog/kaggle:apex_build \
            ...
            --logmsg Recursion-longfinetune-$SIZE-fp16 --nbags 20  --epochs 600 --fold $FOLD  --lr 0.00025  --lrmult 20  --batchsize 16  --workpath scripts/$WDIR  \
            --xtrasteps 200 --expfilter $FILTER --probsname probs_$SIZE  --cutmix_prob 1.0 --precision half  --beta 1.0  --imgpath data/mount/512X512X6/ \
