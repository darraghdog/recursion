...
class ImagesDS(D.Dataset):
    ...
    def __getitem__(self, index):
        ...
        experiment, plate, _ = paths[0].split('/')[-3:]
        stats_dict = statsgrpdf.loc[(experiment, plate)].to_dict()
        statsls = [(stats_dict['Mean'][c], stats_dict['Std'][c]) for c in self.channels]
        img = torch.cat([self._load_img_as_tensor(img_path, seed, mean_, sd_) for ((mean_, sd_), img_path) in zip(statsls, paths)])
        #img = torch.cat([self._load_img_as_tensor(img_path, seed) for img_path in paths])
        exps = self.records[index].experiment.split('-')[0]
        exps = lenc.transform([exps])
        exps = ohc.transform(np.expand_dims(exps, 1)).flatten()


        if self.mode == 'train':
            return img, exps, self.records[index].sirna
        else:
            return img, exps, self.records[index].id_code
...
logger.info('Set up encodess: time {}'.format(datetime.datetime.now().time()))
ohc = OneHotEncoder(sparse=False, dtype=np.float32)
lenc = LabelEncoder()
expsall = train_dfall.experiment.apply(lambda x: x.split('-')[0])
...
for epoch in range(EPOCHS):
    ...
    model.train()
    acc = np.zeros(1)
    for x, exp, y in loader: 
        x = x.to(device)
        exp = exp.cuda() #.to_device()
        optimizer.zero_grad()
        output = model(x, exp)
