...
# Output cosine distance on train
ttsexpdf = pd.DataFrame(ttsexp.astype(np.float32))
ttsexpdf['id_code'] = testdf['id_code'].values
OUTFILE = os.path.join( WORK_DIR, PROBSNAME)
logger.info('Write out {}'.format(OUTFILE))
ttsexpdf.to_csv(OUTFILE,index = False, compression = 'gzip')

# Output cosine distance on train and other experiments in test
EXPERIMENTGROUPS = ['HEPG', 'RPE', 'HUVEC', 'U2OS']
for expgrp in EXPERIMENTGROUPS: 
    print('Run {}'.format(expgrp))
    for exp in dftst[dftst.experiment.str.contains(expgrp)].experiment.unique():
        idxtrn = (dftrn.experiment.str.contains(expgrp))
        idxpsutrn = (dftst.experiment.str.contains(expgrp)) & (dftst.experiment != exp)
        idxpsutst = (dftst.experiment.str.contains(expgrp)) & (dftst.experiment == exp)
        print('Run exp : {}'.format(exp))
        # make data frame
        dftrnpsu = dftst[idxpsutrn]
        dftrnpsu['sirna'] = bootdf.sirna.values[idxpsutrn]
        dftrntmp = pd.concat([dftrn[idxtrn], dftrnpsu], 0)
        dftsttmp = dftst[idxpsutst]
        dftrntmp = dftrntmp.reset_index(drop=True)
        dftsttmp = dftsttmp.reset_index(drop=True)
        # make embeddings
        embtrntmp = np.concatenate((embtrn[idxtrn], embtst[idxpsutrn]), 0)
        embtsttmp = embtst[idxpsutst]
        ttsexp[idxpsutst] = expshotdelta_v5(embtsttmp, embtrntmp, \
                                    dftrntmp.sirna, dftrntmp.experiment, \
                                    dftsttmp.experiment, dist_types=['cosine'])

ttsexpdf = pd.DataFrame(ttsexp.astype(np.float32))
ttsexpdf['id_code'] = testdf['id_code'].values
OUTFILE = os.path.join( WORK_DIR, 'psu_'+ PROBSNAME)
logger.info('Write out {}'.format(OUTFILE))
ttsexpdf.to_csv(OUTFILE,index = False, compression = 'gzip')
