...
bsub  -q lowpriority -gpu "num=$N_GPU:mode=exclusive_process" -app gpu -n =$N_GPU  -env LSB_CONTAINER_IMAGE=darraghdog/kaggle:apex_build \
            -n 1 -R "span[ptile=4]" -o log_train_%J  sh -c "cd /share/dhanley2/recursion/scripts/$WDIR  && python3 trainnopsu.py  \
            --logmsg Recursion-concat-$SIZE-fp16 --nbags 10  --epochs 100 --fold $FOLD  --lr 0.000035 --lrmult 10  --batchsize 16  --workpath scripts/$WDIR  \
