...
test_experiments = dftst.experiment
FINETUNEEXPERIMENTS = ['U2OS']
for expgrp in FINETUNEEXPERIMENTS: 
    print('Run {}'.format(expgrp))
    tstidx = test_experiments.str.contains(expgrp)
    dftrn = loadobj(os.path.join( emb_path, '_df_{}_trn_probs_512_fold5.pk'.format(expgrp)))
    dftst = loadobj(os.path.join( emb_path, '_df_{}_tst_probs_512_fold5.pk'.format(expgrp)))
    embtrn = loadobj(os.path.join( emb_path, '_emb_{}_trn_probs_512_fold5.pk'.format(expgrp)))
    embtst = loadobj(os.path.join( emb_path, '_emb_{}_tst_probs_512_fold5.pk'.format(expgrp)))
    embtrn = embtrn[:,:1108]
    embtst = embtst[:,:1108]        
    ttsexp[tstidx] = expshotdelta_v5(embtst, embtrn, dftrn.sirna, dftrn.experiment, dftst.experiment)
