...
def expshotdelta_v6(embtst, embtrn, sirnas, trnexp, tstexp, dist_types=['cosine', 'l1'], MASK = True):
    ...
    embtst = pd.DataFrame(embtst, index = tstexp)
    embtrn = pd.DataFrame(embtrn, index = sirnas)
    embtrn['experiment'] = trnexp.values#.apply(lambda x: x.split('-')[0]).values
    
    # Mean embedding per experiment and per experiment group for train
    embgrptrnavg = embtrn.groupby(['experiment']).mean()
    embtrn = embtrn.reset_index().groupby(['sirna', 'experiment']).mean()
    # Mean embedding per experiment for test
    embgrptstavg = embtst.reset_index().groupby(['experiment']).mean()
    
    # Get train and test experiment groups and sirnas
    trnexp_grp = embtrn.reset_index()['experiment'].values
    tstexp_grp = embtst.reset_index()['experiment'].values
    sirnas = embtrn.reset_index()['sirna'].values    
...
Cosine Distance
'''
ttsexp = np.zeros((dftst.shape[0], 1108), dtype = np.float32)
ttsexpext = np.zeros((dftst.shape[0], 1108), dtype = np.float32)

FINETUNEEXPERIMENTS = ['U2OS', 'HEPG', 'RPE', 'HUVEC']
for expgrp in FINETUNEEXPERIMENTS: 
    print('Run nearest in group {}'.format(expgrp))
    trnidx = dftrn.experiment.str.contains(expgrp)
    tstidx = dftst.experiment.str.contains(expgrp)
    
    ttsexp[tstidx] = expshotdelta_v5(embtst[tstidx], 
                    embtrn[trnidx], 
                    dftrn[trnidx].sirna, 
                    dftrn[trnidx].experiment, 
                    dftst[tstidx].experiment)
    print('Run nearest in experiment {}'.format(expgrp))
    ttsexpext[tstidx] = expshotdelta_v6(embtst[tstidx], 
                    embtrn[trnidx], 
                    dftrn[trnidx].sirna, 
                    dftrn[trnidx].experiment, 
                    dftst[tstidx].experiment)
...
ttsexpext = pd.DataFrame(ttsexpext.astype(np.float32))
ttsexp['id_code'] = ttsexpext['id_code']  = testdf['id_code'].values
OUTFILEGRP = os.path.join( WORK_DIR,'grp_'+ PROBSNAME)
OUTFILEEXP = os.path.join( WORK_DIR,'exp_'+ PROBSNAME)
logger.info('Write out {}'.format(OUTFILEGRP))
ttsexp.to_csv(OUTFILEGRP,index = False, compression = 'gzip')
ttsexpext.to_csv(OUTFILEEXP,index = False, compression = 'gzip')
