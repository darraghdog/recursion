...
class ImagesDS(D.Dataset):
    ...
    def _get_img_path_negative(self, index, channel):
        experiment, well, plate = self.records[index].experiment, 'B02', self.records[index].plate
        return '/'.join([self.img_dir,self.mode,experiment,f'Plate{plate}',f'{well}_s{self.site}_w{channel}.png'])
        
    def __getitem__(self, index):
        seed = random.randint(1,10000)
        paths = [self._get_img_path(index, ch) for ch in self.channels]
        paths += [self._get_img_path_negative(index, ch) for ch in self.channels]
...
model = DensNet(num_classes=classes, num_channels=12)
model.to(device)
