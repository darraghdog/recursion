...
class ImagesDS(D.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            return img, self.records[index].sirna, self.records[index].weight
        elif self.mode == 'val':
...
bad = '28       459     77      905     474     78      864     336     342     545     825     440     397     805     1055    1056    458     916     454     954     577     227     661     220     363     467     643     1096    955     766     121     175     606     647     752     478     756     81      528     420     127     378     481     611     985     368     504     1107    715     1052    519     1051    1077    91      311     829     290     43      773     830     8       673     974     29      101     526     549     896     99      104     402     553     584     602     1104    203     190     717     999     89      210     295     401     432     677     811     982     1028    23      300     304     617     857     70      742     143     285'
bad = ','.join(bad.split())
bad = set(map(int, bad.split(',') ))
trainfull['weight'] = trainfull['sirna'].apply(lambda x: (1+(x in bad))).astype(np.float32)
validdf['weight'] = validdf['sirna'].apply(lambda x: (1+(x in bad))).astype(np.float32)
...
def custom_loss(data, targets, w):
    ce_loss = nn.CrossEntropyLoss(data, targets)
    return ce_loss * w
...
for epoch in range(EPOCHS):
    ...
    for x, y, w in loader: 
        ...
        if beta > 0 and r < cutmix_prob_warmup:
            ...
            #loss1 = criterion(output, target_a_var) * lam 
            #loss2 = criterion(output, target_b_var) * (1. - lam)
            loss1 = criterion(output, target_a_var) * lam * w
            loss2 = criterion(output, target_b_var) * (1. - lam) + w_cut
            loss = loss1 + loss2 
        else:
            # compute output
            input_var = torch.autograd.Variable(x, requires_grad=True)#.half()
            ...
            output = model(input_var)
            loss = criterion(output, target_var) * w
        loss = loss.mean()
