...
class ImagesDS(D.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #ohc_expt = 2*(self.label_binarizer.transform(self.records[index].expkey).astype(np.float32) - 0.5)
        #ohc_expt = torch.from_numpy(ohc_expt)
        if self.mode in ['train', 'val' ]:
            return img, self.records[index].sirna, ohc_expt
        else:
            return img, self.records[index].id_code, ohc_expt
...
class DensNet(nn.Module):
    def __init__(self, num_classes=1000, num_channels=6):
        ...
        self.classifier1 = nn.Linear(1024+4, num_classes, bias=True)
...
label_binarizer.fit(trainfull.expkey)

#logger.info(label_binarizer.transform(trainfull.expkey[:10]))
