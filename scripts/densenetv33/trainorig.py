...
for epoch in range(EPOCHS):
    ...
    # Gradually add sprinkles and holes
    def train_aug2(p=1.):
        return Compose([HorizontalFlip(), VerticalFlip(),
            Cutout(
                num_holes=int(get_curr_para(epoch, para_min=4, para_max=8, peakepoch=int(EPOCHS)/2)),
                max_h_size=int(get_curr_para(epoch, para_max=32, peakepoch=EPOCHS)),
                max_w_size=int(get_curr_para(epoch, para_max=32, peakepoch=EPOCHS)),
                fill_value=0,
                always_apply=False,
                p=get_curr_para(epoch, para_max=0.5, peakepoch=int(EPOCHS*0.7)),
            ),
            ShiftScaleRotate(shift_limit=0.1, scale_limit=0.1,
                         rotate_limit=30, p=get_curr_para(epoch, para_max=0.5, peakepoch=EPOCHS), border_mode = cv2.BORDER_REPLICATE),], p=p)
    ds = ImagesDS(trainfull, path_img)
    ds_val = ImagesDS(validdf, path_img, mode='val')
    ds_test = ImagesDS(test_df, path_img, mode='test')
    loader = D.DataLoader(ds, batch_size=batch_size, shuffle=True, num_workers=5)
    ...
    cutmix_prob_warmup = get_curr_para(epoch, para_min=0.00, para_max=1.0, peakepoch=int(EPOCHS*0.7))
