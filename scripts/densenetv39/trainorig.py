...
class ImagesDS(D.Dataset):
    ...
    def __getitem__(self, index):
        ...
        stats_dictlog = statsgrpdf.loc[(experiment, plate)].to_dict()
        ...
        imglog1 = self._load_imglog_as_tensor(pathnp1,
                                       stats_dictlog['Mean'], #stats_dict['mean'], 
                                       stats_dictlog['Std'], #stats_dict['std'], stats_dict['illum_correction_function'],
                                       self.transform1)
        imglog2 = self._load_imglog_as_tensor(pathnp2,
                                       stats_dictlog['Mean'], #stats_dict['mean'], 
                                       stats_dictlog['Std'], #stats_dict['std'], stats_dict['illum_correction_function'],
                                       self.transform1)

        imgraw1 = self._load_img_as_tensor(pathnp1, 
                                       stats_dict['mean'], 
                                       stats_dict['std'], 
                                       stats_dict['illum_correction_function'],
                                       self.transform1)
        imgraw2 = self._load_img_as_tensor(pathnp2, 
                                       stats_dict['mean'], 
                                       stats_dict['std'], 
                                       stats_dict['illum_correction_function'],
                                       self.transform1)
        img1 = torch.cat((imgraw1, imglog1))
        img2 = torch.cat((imgraw2, imglog2))
...
class DensNet(nn.Module):
    def __init__(self, num_classes=1000, num_channels=12):
        super().__init__()
