...
bsub  -q lowpriority -gpu "num=$N_GPU:mode=exclusive_process" -app gpu -n =$N_GPU  -env LSB_CONTAINER_IMAGE=docker.optum.com/dhanley2/bert:cgan_build \
            -n 1 -R "span[ptile=4]" -o log_train_%J  sh -c "cd /share/dhanley2/recursion/scripts/$WDIR  && python3 train512.py  \
            --logmsg Recursion-512-origneg-diff --nbags 20  --epochs 50 --fold $FOLD  --lr 0.000025 --lrmult 20  --batchsize 8 --workpath scripts/$WDIR  \
            --cutmix_prob 1.0 --beta 1.0  --imgpath data/mount/512X512X6/ --weightsname weights/pytorch_512_model_densenet$FOLD.bin"
