...
bsub  -q lowpriority -gpu "num=$N_GPU:mode=exclusive_process" -app gpu -n =$N_GPU  -env LSB_CONTAINER_IMAGE=docker.optum.com/dhanley2/bert:cgan_build \
            -n 1 -R "span[ptile=4]" -o log_train_%J  sh -c "cd /share/dhanley2/recursion/scripts/$WDIR  && python3 trainv2.py  \
            --logmsg Recursion-v2-negative-diff --nbags 20  --epochs 60 --fold $FOLD  --lr 0.000025 --lrmult 20  --batchsize 32 --workpath scripts/$WDIR  \
