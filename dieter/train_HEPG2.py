...
parser.add_option('-t', '--datapath', action="store", dest="datapath", help="root directory", default="data/128X128X3")
parser.add_option('-m', '--modelpath', action="store", dest="modelpath", help="root directory", default="dieter/weights/128/1")
...
parser.add_option('-n', '--normfile', action="store", dest="normfile", help="Normalisation File", default="experiment_normalizations_128x128x3.p")
...
class ImageDataLoaderV1:
    ...
    def set_gen(self, batch_size, shuffle=True,aug = None, cast_x = None,cast_y = None, preprocess_x=None, preprocess_y=None):
        ...
        # if self.y is None:
        #     self.y = np.array(self.fns.shape[0] * [self.y])
...
class BaseOptions:
    ...
    def __init__(self):
        ...
        self.input_nc=3# help='# of input image channels: 3 for RGB and 1 for grayscale')
        self.output_nc=3#                            help='# of output image channels: 3 for RGB and 1 for grayscale')
        ...
        self.load_size=256#, help='scale images to this size')
        self.crop_size=256#, help='then crop to this size')
...
def get_fp(image_folder, row):
    ...
    well = row['well']
    side = row['side']
    return image_folder + f'{exp}/Plate{plate}/{well}_s{side}_w.png'
...
# test = pd.read_csv( os.path.join( ROOT, 'data', 'test.csv'))#.iloc[:3000]
train['side'] = 1
train2['side'] = 2
